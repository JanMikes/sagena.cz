...
const NewsArticle: React.FC<NewsArticleProps> = ({
  ...
  if (compact) {
    return (
      <Link href={readMoreUrl} className="group block">
        <article className="relative bg-white border border-gray-200 rounded-xl overflow-hidden group-hover:shadow-lg transition-shadow duration-300">
          {image && (
            <div className="block relative aspect-video bg-gray-200 overflow-hidden">
              <Image
                src={image}
                alt={imageAlt || title}
                fill
          ...
          <div className="p-5 pb-14">
            ...
            <h3 className="text-base font-bold text-primary-600 leading-tight group-hover:text-primary-700 transition-colors line-clamp-2">
              {title}
            </h3>
            {excerpt && (
            ...
            <div className="absolute bottom-0 right-0">
              <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-tl-lg rounded-br-xl bg-[#a0bfdf] group-hover:bg-primary-700 transition-colors">
    ...
    <Link href={readMoreUrl} className="group block">
      <article className="relative bg-white border border-gray-200 rounded-xl overflow-hidden group-hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="p-6 pb-16">
          ...
          <h3 className="text-lg font-bold text-primary-600 mb-4 leading-tight group-hover:text-primary-700 transition-colors">
            {title}
          </h3>
          {excerpt && (
